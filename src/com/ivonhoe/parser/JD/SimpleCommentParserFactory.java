...
public class SimpleCommentParserFactory {
    ...
    public static Parser createParser(int type) {
        Parser parser = null;
        commentType = type;
        if (type == TYPE_ANDAPP_PARSER) {
            JL.d("Create andriod app writer...");
        } else if (type == TYPE_WEB_PARSER) {
            JL.d("Create web org.ivonhoe.parser...");
            parser = new WebCommentParser();
        } else {
            JL.d("Create org.ivonhoe.parser error...");
        }
        return parser;
    }

    public static Writer createWriter(int type, File file) {

        if (type == TYPE_EXCEL_WRITER) {
            JL.d("Create excel writer...");
            return new JDCommentWriter(file);
        } else if (type == TYPE_TEXT_WRITER) {
            JL.d("Create txt writer...");
            return new TextWriter();
        } else {
            JL.d("Create writer error...");
            return null;
        }
    }
